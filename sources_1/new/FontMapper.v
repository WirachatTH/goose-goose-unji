...
module FontMapper(
    input [7:0] char_code,    //รหัส ASCII ที่รับเข้ามา
    input is_small_font,      //0 : แถว Y=150 (ฟ้อนท์ปกติ), 1 : แถว Y=158 (ฟ้อนท์เล็ก)
    
    output reg [9:0] o_sprite_x, 
    output reg [9:0] o_sprite_y
    );

    parameter CHAR_W = 5; //สัดส่วนอักขระ
    parameter CHAR_H = 7;

    parameter ROW_LOWER_Y = 132;   //a - z
    parameter ROW_UPPER_Y = 141;   //A - Z
    parameter ROW_NUM1_Y  = 150;   //1-0... ฟ้อนท์ปกติ
    parameter ROW_NUM2_Y  = 158;   //1-0... ฟ้อนท์เล็ก

    always @(*) begin
        o_sprite_x = 0;
        o_sprite_y = 0;

        //ตัวพิมพ์เล็ก (a-z) -> Y = 132
        if (char_code >= "a" && char_code <= "z") begin
            o_sprite_y = ROW_LOWER_Y;
            //พิกัด x บน sprite a=0, b=5, c=10...
            o_sprite_x = (char_code - "a") * CHAR_W;
        end
        
        //ตัวพิมพ์ใหญ่ (A-Z) -> Y = 141
        ...
        //ตัวเลขและสัญลักษณ์ -> Y = 150 หรือ 158
        else begin
            //เลือกพิกัด Y ตาม Input
